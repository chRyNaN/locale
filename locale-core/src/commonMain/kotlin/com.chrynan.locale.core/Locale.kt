...
interface Locale {
    ...
    companion object {
         ...
         * character.
         */
        const val VARIANT_SEPARATOR = "_"
    }
}

/**
 * Converts the provided [languageTag] into a [Locale] or throws an exception if it could not be converted into a
 * [Locale].
 *
 * **Note:** Some platform implementations may be more lenient that others and will try to create a [Locale] removing
 * any unsupported or unmatched values. This logic is up to the implementing class.
 */
expect fun Locale.Companion.parse(languageTag: String): Locale

/**
 * Attempts to parse the provided [languageTag] into a [Locale] using the [parse] function, or `null` if it cannot be
 * converted into a [Locale].
 */
inline fun Locale.Companion.parseOrNull(languageTag: String): Locale? =
    try {
...
/**
 * Retrieves the current default [Locale] for this device and platform. It is dependent on the implementation on how
 * this value is retrieved and may differ between implementations and platforms.
 */
 ...
 */
inline fun Locale.Companion.fromPartsOrNull(
    language: String? = null,
    region: String? = null,
    script: String? = null,
    variants: List<String> = emptyList()
 ...
 */
operator fun Locale.component1(): String? = language

/**
 * Retrieves the [Locale.region] component.
 */
operator fun Locale.component2(): String? = region

/**
 * Retrieves the [Locale.script] component.
 */
operator fun Locale.component3(): String? = script

/**
 * Retrieves the [Locale.variants] component.
 */
operator fun Locale.component4(): List<String> = variants
